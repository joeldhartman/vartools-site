...
| `"mergepeakdf" < "transit" mult \| factor >` | Set the frequency resolution `Df` used to decide whether two spectrum peaks are the same detection. By default `Df = 1/T` (the Rayleigh resolution, `T` = time baseline), which is appropriate for a sinusoid but tends to over-merge transit peaks (a box transit of fractional width `q` is resolved on the finer scale `q/T`). `"transit" mult` sets `Df = mult·q/T` using the per-candidate fitted transit width `q` (a `mult` of order a few is recommended); a bare number sets `Df = factor/T` (`mergepeakdf 1.0` reproduces the default). The reported peaks are guaranteed to be separated by at least `Df`: a lower-S/N peak left within `Df` of a stronger one is dropped and its slot back-filled with the next distinct peak. Note that in `transit` mode `Df = mult·q/T` uses the fitted box width `q` (typically well below 1), so whenever `mult·q < 1` the resolution is **finer** than the default `1/T` and closely-spaced peaks that a `1/T` merge would combine into one detection are reported separately; use the default, or a `factor ≥ 1`, for at-least-Rayleigh (`1/T`) separation. |
